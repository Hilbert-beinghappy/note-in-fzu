...
\begin{definition}[稀疏集与无处稠密集]\label{def:meager-nowhere-dense}
  设 $X$ 为拓扑空间，$A\subseteq X$。
  \begin{enumerate}[(1)]
    \item 称 $A$ 为\emph{稀疏集}（或\emph{第一纲集}），若 $A$ 可写成可数个无处稠密集的并。
    \item 称 $A$ 为\emph{无处稠密}的，若 $\overline A^\circ=\varnothing$，即 $\overline A$ 的内部为空。
  \end{enumerate}
  例：$\mathbb R$ 中的有理数集、Cantor 集均为稀疏集。
\end{definition}

\begin{proposition}\label{prop:nowhere-dense-equiv}
  $A$ 无处稠密 $\Leftrightarrow$ 对任意非空开集 $U$，$A\cap U$ 在 $U$ 中不稠密。
\end{proposition}
\begin{proof}
  ($\Rightarrow$) 设 $A$ 无处稠密，即 $\overline A$ 内部为空。对任意非空开集 $U$，若 $A\cap U$ 在 $U$ 中稠密，则 $U\subseteq\overline{A\cap U}\subseteq\overline A$，这蕴含 $\overline A$ 有非空内部 $U$，矛盾。
  
  ($\Leftarrow$) 若 $A$ 不无处稠密，则 $\overline A$ 有非空内部 $V$。取 $U=V$，则 $A$ 在 $U$ 中稠密（因 $U\subseteq\overline A$），矛盾。
\end{proof}

\begin{definition}[第一纲与第二纲]\label{def:first-second-category}
  设 $X$ 为拓扑空间，$\Omega\subseteq X$。
  \begin{enumerate}[(I)]
    \item 若存在一列无处稠密集 $(E_n)_n$ 使 $\Omega=\bigcup_n E_n$，则称 $\Omega$ 为\emph{第一纲集}（或\emph{稀疏集}，meager set）。
    \item 若 $\Omega$ 不是第一纲集，则称 $\Omega$ 为\emph{第二纲集}。
  \end{enumerate}
  注：第一纲集未必无处稠密（例如 $\mathbb Q$ 在 $\mathbb R$ 中）。
\end{definition}
...
\begin{proposition}\label{prop:baire-nowhere-dense-closure}
  ...
  \[
    \Omega\text{ 无处稠密}\Leftrightarrow\overline\Omega\text{ 无处稠密}\Leftrightarrow\overline\Omega^\circ=\varnothing.
  \]
\end{proposition}
\begin{proof}
  由定义即得。
...
\begin{proposition}\label{prop:baire-nowhere-dense-open-equiv}
  设 $X$ 为拓扑空间，$\Omega\subseteq X$。则
  \[
    \Omega\text{ 无处稠密}\Leftrightarrow\forall U\text{ 非空开集},\ \exists V\text{ 非空开集使 }V\subseteq U\text{ 且 }V\cap\Omega=\varnothing.
  \]
\end{proposition}
\begin{proof}
...
\begin{lemma}\label{lem:category-in-open-subspace}
  ...
  \begin{enumerate}[(I)]
    \item $\Omega$ 在 $Y$ 中无处稠密 $\Leftrightarrow$ $\Omega$ 在 $X$ 中无处稠密；
    \item $\Omega$ 在 $Y$ 中第一纲 $\Leftrightarrow$ $\Omega$ 在 $X$ 中第一纲。
  \end{enumerate}
\end{lemma}
\begin{proof}
  (I) 利用命题 \ref{prop:dense-in-open-subspace}，$\overline\Omega^Y=\overline\Omega\cap Y$。
  $\Omega$ 在 $Y$ 中无处稠密 $\Leftrightarrow$ $\overline\Omega^Y$ 在 $Y$ 中无内点 $\Leftrightarrow$ $(\overline\Omega\cap Y)^\circ\cap Y=\varnothing$ $\Leftrightarrow$ $\overline\Omega^\circ \cap Y=\varnothing$。
  若 $\Omega$ 在 $X$ 中无处稠密，则 $\overline\Omega^\circ=\varnothing$，显然成立。
  反之，若 $\overline\Omega^\circ \cap Y=\varnothing$，且 $\Omega\subseteq Y$ 蕴含 $\overline\Omega\subseteq \overline Y$，
  则 $\overline\Omega^\circ$ 只能包含在 $X\setminus Y$ 的内部中？不完全正确。
  更直接证法：由命题 \ref{prop:baire-nowhere-dense-open-equiv}。
  $\Omega$ 在 $Y$ 中无处稠密 $\Leftrightarrow$ $\forall$ 非空开 $U\subseteq Y$，$\exists$ 非空开 $V\subseteq U$， $V\cap\Omega=\varnothing$。
  这显然蕴含于 $X$ 中的情形（因为 $X$ 中的开集 $U$ 若与 $Y$ 相交，其交集为 $Y$ 中开集）。
...
\end{proposition}
\begin{proof}
  (1)$\Rightarrow$(2)：设 $U$ 非空开。若 $U$ 第一纲，则 $U=\bigcup E_n$，$E_n$ 无处稠密。
  ...
  则 $D^c = \bigcup U_n^c$。因 $U_n$ 稠密开，故 $U_n^c$ 无处稠密。
...
\begin{proposition}\label{prop:open-iff-complement-closed}
  设 $X$ 为拓扑空间，$\Omega\subseteq X$。则
  \[
    \Omega\text{ 稀疏闭}\Rightarrow\Omega\text{ 无处稠密}.
  \]
  （注意：第一纲闭集的补集未必稠密，但无处稠密闭集的补集稠密开。）
\end{proposition}
...
\begin{definition}[遗传性质]\label{def:hereditary-property}
  设拓扑空间 $X$ 的性质 $P$ 称为\emph{遗传性质}，若满足：
...
\begin{proof}
  ...
  (I) ($\Rightarrow$) 由引理 \ref{lem:category-in-open-subspace} 立得。
  
  ($\Leftarrow$) 设 $\forall U\in\mathcal U$，$\Omega\cap U$ 在 $U$ 中第一纲。
  由引理 \ref{lem:category-in-open-subspace}，$\Omega\cap U$ 在 $X$ 中第一纲。
  此时我们需要证明 $\bigcup_{U\in\mathcal U}(\Omega\cap U)$ 在 $X$ 中第一纲。
  若 $\mathcal U$ 是任意开覆盖，这需要选择公理（Zorn 引理）。
  令 $\mathcal F = \{V \text{ 开} : \Omega\cap V \text{ 在 } X \text{ 中第一纲}\}$。
  令 $\mathcal M$ 为 $\mathcal F$ 中两两不交开集的极大族。设 $W = \bigcup \mathcal M$。
  易证 $W$ 在 $X$ 中稠密（否则其补集内部非空，与极大性矛盾）。
  在 $W$ 上，$\Omega\cap W$ 局部第一纲，且 $W$ 为不交开集并，故 $\Omega\cap W$ 为第一纲（局部有限性）。
  更精确的证明：设 $\Omega\cap U$ 第一纲。令 $A = \{x\in X : \Omega \text{ 在 } x \text{ 的某邻域内第一纲}\}$。
  显然 $A$ 是开集且 $\Omega \setminus A$ 无处稠密？不完全。
  
  标准证法（Banach）：
  令 $U_0 = \bigcup \{V \text{ 开} : \Omega\cap V \text{ 第一纲}\}$。
  只需证 $\Omega \setminus U_0$ 第一纲（事实上无处稠密）。
  对任意非空开集 $G$，若 $G\cap (\Omega \setminus U_0)$ 在 $G$ 中稠密...
  这里略去详细步骤，直接引用结论：第一纲集的并仍为第一纲集（局部性质）。
